    ...
    def validate_confirm(form, field):
        ...
        if password_value != confirm_value:
            raise ValidationError(
                "Please type in the same password as in the password field."
            )
...
class ChildForm(Form):
    ...
    gender = SelectField(
        "gender", validators=[Optional()], choices=[(x, x) for x in ["Male", "Female"]]
    )
    first_name = StringField("first_name", validators=[DataRequired(), Length(max=15)])
    last_name = StringField("last_name", validators=[DataRequired(), Length(max=15)])
    nick_name = StringField("nick_name", validators=[Length(max=15)])
    birth_date = DateField("birth_date", validators=[DataRequired()])
    birth_date_accuracy = SelectField(
        "birth_date_accuracy",
        validators=[Optional()],
        choices=[(x, x) for x in [EXACT, ESTIMATED]],
    )
    nationality = SelectField(
        "nationality",
        validators=[Optional()],
        choices=[(x, x) for x in [HAITIAN, DOMINICAN, HAITIAN_DOMINICAN]],
    )
    ...
    number_of_siblings = IntegerField(
        "number_of_siblings",
        validators=[NumberRange(min=0, max=max_number_of_siblings)],
    )
    siblings_in_project = StringField("siblings_in_school", validators=[Length(max=40)])
    school_class = SelectField(
        "school_class",
        validators=[Optional()],
        choices=[
            (x, x)
    ...
    school_attendance = SelectField(
        "school_attendance",
        validators=[Optional()],
        choices=[(x, x) for x in ["Good", "Intermediate", "Bad"]],
    )
    projects = SelectMultipleField()
    volunteer_task = StringField("volunteer_task")
    situation = StringField("situation")
    godparent_status = SelectField(
        "godparent_status",
        validators=[Optional()],
        choices=[(x, x) for x in [SEARCHING_GODPARENT, CHILD_HAS_GODPARENT, NO_NEED]],
    )
